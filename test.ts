...
async function _greet(name: string): Promise<string> {
  const prompt = `Please greet ${name} in Hindi.`;
  ...
  return result.value;
    ...
    // console.error("Error parsing response for variable 'greet':", e);
...
graph.node("main", async (state) => {
    ...
    const name = await _builtinInput("Enter your name: ");


const greet = await _greet(name);

console.log(greet)
return { ...state, data: greet}
